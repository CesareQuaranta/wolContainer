...
public class SolarSystemPhisycs implements iPhisycs<Planetoid> {
	...
	public Force calculateAttraction(Planetoid subject,Planetoid object) {
		...
		double difX=(objectPosition.getX()-subjectPosition.getX())/radiusGravity;
		double difY=(objectPosition.getY()-subjectPosition.getY())/radiusGravity;
		double difZ=(objectPosition.getZ()-subjectPosition.getZ())/radiusGravity;
	...
	protected void generateAcceleration(Planetoid planet){
		...
		for (iPower curPower:attractions){
			if (curPower instanceof Force){
				...
				Vector forceVector=curAttraction.getVector().multiply(1/curAttraction.getIntensity());
		...
		if (a>1){
			acceleration=new Acceleration<Planetoid>(planet,1,accelerationVector);
		}else{
			double dFuture=1/a;
			...
			acceleration=new Acceleration<Planetoid>(planet,future,newAccelerationVector);
		}
	...
	public void processEvent(iEvent<Planetoid> event) {
		if (event instanceof Acceleration&&((Acceleration<Planetoid>) event).getDelay()==0){
			...
			Set<LatentEffect<Entity>> latents=planet.getLatentEffects(Movement.class);
			curVelocity.sum(acceleration.getVector());
			long future=1;
			if (curVelocity.getLenght()<1){
				//recalculate vector& future
			}
			Movement<Planetoid> movement=new Movement<Planetoid>(planet,future,curVelocity);
			if (latents.isEmpty()){
				planet.addLatentEffect((LatentEffect)movement);
				fireEvent(movement);
			}else{//TODO Karma transformation, Remove event, recalculate, insert
	...
	protected void procesSeeds(List<Seed<Planetoid>> seeds) {
		...
		for (Seed<Planetoid> curSeed : seeds) {
			...
			double distanceX = subjectPosition.getX() - objectPosition.getX();
			double distanceY = subjectPosition.getY() - objectPosition.getY();
			double distanceZ = subjectPosition.getZ() - objectPosition.getZ();
			...
			Vector vettore = new Vector(distanceX / minDistance, distanceY
					/ minDistance, distanceZ / minDistance);
